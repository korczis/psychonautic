+++
title = "Substance"
description = "Prehled syntetickych a semisyntetickych psychoaktivnich latek"
weight = 10
sort_by = "weight"
insert_anchor_links = "right"
+++

# Psychoaktivni substance

Tato sekce obsahuje informace o **syntetickych** a **semisyntetickych** psychoaktivnich latkach, ktere nejsou primarne klasifikovany jako alkaloidy. Zahrnuje disociativa, designer drogy a farmaceuticke pripravky.

---

## Disociativa

Latky pusobici primarne jako antagoniste [NMDA receptoru](@/receptors/nmda.md):

| Latka | Typ | Hlavni mechanismus |
|-------|-----|-------------------|
| [DXM](@/substances/dxm.md) | OTC antitusikum | NMDA antagonista, SERT inhibitor |
| [Esketamin](@/substances/esketamine.md) | FDA schvalene antidepresivum | NMDA antagonista |
| [MXE](@/substances/mxe.md) | Designer disociativ | NMDA antagonista, SERT inhibitor |
| [PCP](@/substances/pcp.md) | Historicke anestetikum | Potentni NMDA antagonista |

---

## Klasifikace podle mechanismu

### NMDA antagoniste

```
NMDA RECEPTOROVÁ BLOKÁDA
         |
    +----+----+----+----+
    |    |    |    |    |
    v    v    v    v    v
  PCP   MXE  DXM  Ketamin  Esketamin
 (50nM) (300) (2000) (500-1000) (300-500)

Ki hodnoty (nM) - nizsi = vyssi afinita
```

### Serotoninergni komponenta

| Latka | SERT Ki (nM) | Serotoninergni riziko |
|-------|--------------|----------------------|
| **DXM** | 23-40 | VYSOKE |
| **MXE** | 500-1000 | VYSOKE |
| **Ketamin** | >10,000 | Nizke |
| **PCP** | 2,234 | Stredni |

---

## Trvani ucinku

```
SROVNANI TRVANI DISOCIATIV

PCP       ████████████████████████████ (4-8+ h)
MXE       ███████████████████ (3-5 h)
DXM       ████████████████████ (4-6 h p.o.)
Ketamin   ██████ (1-2 h)
Esketamin ██████ (1-2 h)

Polocas:
PCP: 7-46 h (!!)
MXE: 3-6 h
DXM: 2-4 h (DXO 3-6 h)
Ketamin: 2-3 h
```

---

## Pravni status (CR)

| Latka | Status CR | Poznamka |
|-------|-----------|----------|
| **DXM** | Volne prodejne | OTC antitusikum |
| **Esketamin** | Rx | Registrovany lek (Spravato) |
| **MXE** | Sede zona | Potencialne analogova klauze |
| **PCP** | Kontrolovana | Psychotropni latka |
| **Ketamin** | Rx | Navykova latka |

---

## Bezpecnostni upozorneni

**SEROTONIN SYNDROM**: DXM a MXE jsou potentni SERT inhibitory. Kombinace s SSRI, SNRI nebo MAOI je **KONTRAINDIKOVANA** a muze byt **SMRTELNA**.

**PROLONGOVANE UCINKY**: PCP ma extremne dlouhy polocas (az 46 hodin). Intoxikace muze trvat dny.

---

## Reference

1. Lodge, D. & Mercier, M.S. (2015). *Ketamine and phencyclidine: the good, the bad and the unexpected*. British Journal of Pharmacology.

2. Morris, H. & Wallach, J. (2014). *From PCP to MXE: A comprehensive review of the non-medical use of dissociative drugs*. Drug Testing and Analysis.

---

Viz take:
- [NMDA receptor](@/receptors/nmda.md) - Hlavni cil disociativ
- [Ketamin](@/alkaloids/ketamin.md) - Arylcyklohexylamin
- [Sigma-1 receptor](@/receptors/sigma-1.md) - Sekundarni cil

<- Zpet na [Hlavni stranku](@/_index.md)
